...
class Options extends AbstractHookProvider {
	 ...
	 * @return mixed
	 */
	public function get( $option_id, $default = null, $option_details = null ) {
	 ...
	 */
	public function get_details( $option_id, $minimal_details = false, $skip_cache = false ) {
		if ( empty( $option_id ) ) {
			return false;
		}
	 ...
	 *
	 */
	protected function get_wpoptions_value( $option_id, $setting_id ) {
		global $wp_customize;
	 ...
	 *
	 */
	protected function get_thememod_value( $option_id, $setting_id ) {
		global $wp_customize;
	 ...
	 * @return bool
	 */
	public function skip_dev_mode_force_defaults( $option_id, $option_config = null ): bool {
	 ...
	 * @return string
	 */
	public function get_options_key( $skip_cache = false ): string {
	...
	protected function load_customizer_config( $skip_cache = false ): array {
		...
		if ( true === $skip_cache || false === $data || false === $expire_timestamp || $expire_timestamp < time() ) {
			...
			if ( ! is_array( $data ) ) {
				\_doing_it_wrong( __METHOD__, esc_html__( 'The Style Manager fields configuration should be an array. Please check the filters that are hooked into \'style_manager/filter_fields\'.', 'style-manager' ), null );
			...
			// Make sure that we have an array.
			if ( ! is_array( $data ) ) {
				\_doing_it_wrong( __METHOD__, esc_html__( 'The Style Manager fields configuration should be an array. Please check the filters that are hooked into \'style_manager/final_config\'.', 'style-manager' ), null );
  ...
  */
 protected function prevent_deletion_on_save( $delete, $field ) : bool {
