...
class Fonts extends AbstractHookProvider {
	 ...
	 * @return array
	 */
	public function standardize_global_config( $config ): array {
	 ...
	 * @return array
	 */
	public function standardizeFontValue( $value, $fontConfig = [] ): array {
		if ( empty( $value ) ) {
			return [];
		}
  ...
  */
 public function getFontDetails( $font_family, $font_type = '' ): array {
		if ( empty( $font_type ) ) {
	...
	function output_third_party_fonts_select_options_group( $active_font_family, $current_value ) {
		...
		if ( ! empty( $this->third_party_fonts ) ) {
			$group_label = apply_filters( 'style_manager/third_party_font_group_label', esc_html__( 'Third-Party Fonts', 'style-manager' ), $active_font_family, $current_value );
	...
	public function output_cloud_fonts_select_options_group( $active_font_family, $current_value ) {
		...
		if ( ! empty( $this->cloud_fonts ) ) {
			echo '<optgroup label="' . esc_html__( 'Cloud Fonts', 'style-manager' ) . '">';
	...
	public function output_system_fonts_select_options_group( $active_font_family, $current_value ) {
		...
		if ( ! empty( $this->system_fonts ) ) {
			...
			echo '<optgroup label="' . esc_attr__( 'System fonts', 'style-manager' ) . '">';
	...
	public function output_google_fonts_select_options_group( $active_font_family, $current_value ) {
		...
		if ( ! empty( $this->google_fonts ) ) {
			...
			echo '<optgroup class="google-fonts-opts-placeholder" label="' . esc_attr__( 'Google fonts', 'style-manager' ) . '"></optgroup>';
		}
	...
	protected function getGoogleFontsSelectOptions() {
		...
		if ( $this->plugin_settings->get( 'typography_group_google_fonts', 'yes' ) ) {
			...
			foreach ( $grouped_google_fonts as $group_name => $group_fonts ) {
				...
				echo '<optgroup label="' . sprintf( esc_attr__( 'Google fonts %s', 'style-manager' ), $group_name ) . '">';
			...
			echo '<optgroup label="' . esc_attr__( 'Google fonts', 'style-manager' ) . '">';
			foreach ( $this->get_google_fonts() as $font_details ) {
				$this->output_font_family_option( $font_details['family'] );
			}
			echo '</optgroup>';
	 ...
	 * @return array
	 */
	public function getFontDefaultsValue( $font_family ): array {
		if ( empty( $font_family ) ) {
			return [];
		}
...
<?php
	 ...
	 */
	protected function getFontStyle( $fontConfig ): string {
	 ...
	 * @return array
	 */
	protected function getCSSValue( $value, $fontConfig ): array {
		$cssValue = [];
...
if (typeof WebFont !== 'undefined') {
	...
	public function add_to_localized_data( $localized ) {
		...
		$localized['l10n']['fonts'] = [
			'familyPlaceholderText' => esc_html__( 'Select a font family', 'style-manager' ),
			'variantAutoText'       => esc_html__( 'Auto', 'style-manager' ),
		];
	 ...
	 * @return bool
	 */
	protected function isCSSPropertyAllowed( $property, $allowedProperties = false ): bool {
	 ...
	 */
	public function determineFontType( $fontFamily ): string {
