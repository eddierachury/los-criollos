...
class Customize extends AbstractHookProvider {
	 ...
	 * @return array
	 */
	protected function setup_sm_section_base_config( $config ): array {
	 ...
	 * @return array
	 */
	protected function reorganize_sections( $config ): array {
		// If there is no Style Manager support, bail early.
		if ( ! $this->is_supported() ) {
			return $config;
		}
		...
		if ( empty( $config['panels']['style_manager_panel'] ) ) {
			$sm_panel_config = [
				...
				'title'                    => esc_html__( 'Style Manager', 'style-manager' ),
				'description'              => wp_kses_post( __( '<strong>Style Manager</strong> is an intuitive system to help you change the look of your website and make an excellent impression.', 'style-manager' ) ),
				'sections'                 => [],
		...
		$theme_options_panel_config = [
			...
			'title'       => esc_html__( 'Theme Options', 'style-manager' ),
			'description' => esc_html__( 'Advanced options to change your site look-and-feel on a detailed level.', 'style-manager' ),
			'sections'    => [],
		];
	 ...
	 */
	protected function reorganize_direct_sections_and_controls( $wp_customize ) {
			...
			if ( false !== strpos( $section->id, 'theme_options' ) ) {
				...
				if ( false === $general_section ) {
					...
					$general_section = $wp_customize->add_section( 'theme_options[general]', [
						'title'    => esc_html__( 'General', 'style-manager' ),
	 ...
	 * @param \WP_Customize_Manager $wp_customize
	 */
	protected function remove_switch_theme_panel( $wp_customize ) {
		// If there is no Style Manager support, bail.
		if ( ! $this->is_supported() ) {
			return;
		}
	 ...
	 * @param \WP_Customize_Manager $wp_customize
	 */
	protected function remove_widgets_panel( $wp_customize ) {
		// If there is no Style Manager support, bail.
		if ( ! $this->is_supported() ) {
			return;
		}
	 ...
	 */
	protected function general_reorganization_of_customize_sections( $wp_customize ) {
		...
		$wp_customize->get_section( 'title_tagline' )->title = esc_html__( 'Site Identity', 'style-manager' );
	}
	 ...
	 * @return array
	 */
	protected function filter_based_on_theme_type( $config ): array {
								...
								<div class="modal-header">
									<button type="button" class="close icon media-modal-close" data-dismiss="modal"
									        aria-label="Close"><span class="media-modal-icon"><span
												class="screen-reader-text"><?php esc_html_e( 'Close media panel', 'style-manager' ); ?></span></span></button>
									<!-- <a href="#" class="close button button--naked gray" data-dismiss="modal" aria-label="Close">Close</a> -->
								</div>
								<div class="modal-body full">
									<div class="box box--large">
										<div class="first-step">
											<h2 class="modal-title"><?php esc_html_e( 'How would you rate your experience in finding the right colors for your site?', 'style-manager' ); ?></h2>
											<div class="scorecard">
												<span><?php
													/* translators: The Style Manager lowest rating.  */
													esc_html_e( 'Poor', 'style-manager' ); ?></span>
												<label>
												...
												<span><?php
													...
													esc_html_e( 'Great', 'style-manager' ); ?></span>
											</div>
										</div>
										<div class="second-step hidden">
											<p><strong><?php
													/* translators: %s: The selected star rating */
													printf( esc_html__( 'What points along the way made this a %s* experience for you?', 'style-manager' ), '<span class="rating-placeholder">5</span>'); ?></strong><br>
												<?php esc_html_e( 'We are counting on your insights to guide us in doing better 🙏', 'style-manager' ); ?></p>
											<div class="not-floating-labels">
												<div class="form-row field">
												<textarea name="message"
												          placeholder="<?php esc_html_e( 'Describe your experience in customizing your site colors..', 'style-manager' ); ?>"
												          id="style-manager-user-feedback-message" rows="6"
												          oninvalid="this.setCustomValidity('<?php esc_html_e( 'May we have a little more info about your experience?', 'style-manager' ); ?>')"
												          oninput="setCustomValidity('')" required></textarea>
												</div>
											</div>
											<button id="style-manager-user-feedback_btn" class="button"
											        type="submit"><?php esc_html_e( 'Send us your insights', 'style-manager' ); ?></button>
										</div>
										<div class="thanks-step hidden">
											<h3 class="modal-title"><?php esc_html_e( 'Thank you so much for your feedback!', 'style-manager' ); ?></h3>
											<p><?php esc_html_e( 'It means the world to us as we strive to constantly push the limits and aim higher. Stay awesome! 🤗', 'style-manager' ); ?></p>
											<p><em><?php
													/* translators: %s: Pixelgrade */
													printf( esc_html__( 'The %s Team', 'style-manager' ), 'Pixelgrade'); ?></em></p>
										</div>
										<div class="error-step hidden">
											<h3 class="modal-title"><?php esc_html_e( 'We\'ve hit a snag!', 'style-manager' ); ?></h3>
											<p><?php esc_html_e( 'We couldn\'t record your feedback and we would truly appreciate it if you would try it again at a latter time. Stay awesome! 🤗', 'style-manager' ); ?></p>
										</div>
									</div>
								</div>
