 ...
 protected $localized = [];
	...
	protected function setup() {
		...
		$this->localized = [
			...
			'l10n'   => [
				'panelResetButton'           => esc_html__( 'Panel\'s defaults', 'style-manager' ),
				'sectionResetButton'         => esc_html__( 'Reset All Options for This Section', 'style-manager' ),
				'resetGlobalConfirmMessage'  => wp_kses_post( __( 'Do you really want to reset to defaults all the fields? Watch out, this will reset all your Style Manager options and will save them!', 'style-manager' ) ),
				'resetPanelConfirmMessage'   => wp_kses_post( __( 'Do you really want to reset the settings in this panel?', 'style-manager' ) ),
				'resetSectionConfirmMessage' => wp_kses_post( __( 'Do you really want to reset the settings in this section?', 'style-manager' ) ),
			],
		];
	 ...
	 * @return array
	 */
	protected function prevent_changeset_save_in_devmode( $data ): array {
		// Get the options key.
			...
			(function ($, exports, wp) {
				...
				wp.customize.bind('ready', function () {
					wp.customize.notifications.add('style_manager_force_defaults', new wp.customize.Notification(
						...
						{
							type: 'warning',
							message: '<?php echo \wp_kses_post( __( '<strong>Style Manager: Development Mode</strong><p>All options are switched to default. While they are changing in the live preview, they will not be kept when you hit publish.</p>', 'style-manager' ) ); ?>'
						}
					))
				})
			})(jQuery, window, wp)
	 ...
	 */
	protected function process_customizer_config( $wp_customize ) {
		...
		if ( $this->plugin_settings->get( 'enable_reset_buttons' ) ) {
			...
			$reset_section_settings = [
				'title'      => esc_html__( 'Style Manager Toolbox', 'style-manager' ),
	...
	protected function register_section( $panel_id, $section_key, $options_name, $section_config, $wp_customize ) {
		...
		if ( ! $wp_customize->get_section( $section_id ) ) {
			...
			$section_args = wp_parse_args( $section_config, [
				...
				'capability'         => 'edit_theme_options',
				'theme_supports'     => '',
				'title'              => esc_html__( 'The Title Section is required', 'style-manager' ),
				'description'        => '',
				'type'               => 'default',
	...
	protected function register_field( $section_id, $setting_id, $field_config, $wp_customize ) {
		...
		$this_control = new $control_class_name($wp_customize, $setting_id . '_control', $control_args);
	 ...
	 *
	 */
	protected function maybe_remove_default_sections( $wp_customize ) {
		global $wp_registered_sidebars;
	 ...
	 * @param \WP_Customize_Manager $wp_customize
	 */
	protected function maybe_process_config_extras( $wp_customize ) {
	 ...
	 * @return array
	 */
	protected function default_options( $config ): array {
		...
		$config['sections'] = [
			 ...
			 */
			'presets_section'     => [
				'title'   => esc_html__( 'Style Presets', 'style-manager' ),
				'options' => [
					'theme_style' => [
						'type'         => 'preset',
						'label'        => esc_html__( 'Select a style:', 'style-manager' ),
						'desc'         => esc_html__( 'Conveniently change the design of your site with built-in style presets. Easy as pie.', 'style-manager' ),
						'default'      => 'royal',
						'choices_type' => 'awesome',
						'choices'      => [
							'royal'  => [
								'label'   => esc_html__( 'Royal', 'style-manager' ),
								'preview' => [
									'color-text'       => '#ffffff',
							...
							'lovely' => [
								'label'   => esc_html__( 'Lovely', 'style-manager' ),
								'preview' => [
									'color-text'       => '#ffffff',
							...
							'queen'  => [
								'label'   => esc_html__( 'Queen', 'style-manager' ),
								'preview' => [
									'color-text'       => '#fbedec',
							...
							'carrot' => [
								'label'   => esc_html__( 'Carrot', 'style-manager' ),
								'preview' => [
									'color-text'       => '#ffffff',
							...
							'adler'  => [
								'label'   => esc_html__( 'Adler', 'style-manager' ),
								'preview' => [
									'color-text'       => '#fff',
							...
							'velvet' => [
								'label'   => esc_html__( 'Velvet', 'style-manager' ),
								'preview' => [
									'color-text'       => '#ffffff',
			 ...
			 */
			'colors_section'      => [
				'title'   => esc_html__( 'Colors', 'style-manager' ),
				'options' => [
					'links_color'    => [
						'type'    => 'color',
						'label'   => esc_html__( 'Links Color', 'style-manager' ),
						'live'    => true,
						'default' => '#6c6e70',
						'css'     => [
							[
					...
					'headings_color' => [
						'type'    => 'color',
						'label'   => esc_html__( 'Headings Color', 'style-manager' ),
						'live'    => true,
						'default' => '#0aa0d9',
						'css'     => [
							[
					...
					'body_color'     => [
						'type'    => 'color',
						'label'   => esc_html__( 'Body Color', 'style-manager' ),
						'live'    => true,
						'default' => '#2d3033',
						'css'     => [
							[
			 ...
			 */
			'typography_section'  => [
				'title'   => esc_html__( 'Fonts', 'style-manager' ),
				'options' => [
					'headings_font' => [
						'type'        => 'font',
						'label'       => esc_html__( 'Headings', 'style-manager' ),
					...
					'body_font'     => [
						'type'        => 'font',
						'label'       => esc_html__( 'Body Text', 'style-manager' ),
						'default'     => 'Lato',
						'selector'    => 'html body',
						'recommended' => [
							'Lato',
			 ...
			 */
			'backgrounds_section' => [
				'title'   => esc_html__( 'Backgrounds', 'style-manager' ),
				'options' => [
					'page_background' => [
						'type'    => 'color',
						'label'   => esc_html__( 'Page Background', 'style-manager' ),
						'live'    => true,
						'default' => '#ffffff',
						'css'     => [
							[
			 ...
			 */
			'layout_options'      => [
				'title'   => esc_html__( 'Layout', 'style-manager' ),
				'options' => [
					'site_title_size'      => [
						'type'        => 'range',
						'label'       => esc_html__( 'Site Title Size', 'style-manager' ),
						'live'        => true,
						'input_attrs' => [
							'min'          => 24,
							'max'          => 100,
