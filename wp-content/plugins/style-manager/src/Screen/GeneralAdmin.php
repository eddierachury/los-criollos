...
class GeneralAdmin extends AbstractHookProvider {
	 ...
	 */
	protected function maybe_show_notice_to_migrate_when_child_theme( $old_theme_name, $old_theme ) {
	...
	function child_theme_migrate_theme_mods_notice() {
		...
		<div class="style-manager-notice__container updated notice fade is-dismissible">
			<h3><?php
				...
				echo sprintf( __( 'You have activated a child theme for "%s". Good for you!', 'style-manager' ), $parent_theme->get('Name') );
			?></h3>
			<p>
				<?php echo wp_kses_post( __( 'If you have already <strong>set up things in the Customizer,</strong> you may want to <strong>keep those customizations</strong> so you don\'t start over.', 'style-manager' ) ); ?>
			</p>
			<p>
				<?php echo wp_kses_post( __( 'So, the question is simple: <strong>would you like to migrate all theme-specific options (theme mods) from the parent theme to the child one?</strong>', 'style-manager' ) ); ?>
			</p>
			<p>
				<?php echo wp_kses_post( __( 'All parent theme customizations will remain in place, while those of the active child theme will be overwritten, if any.', 'style-manager' ) ); ?>
			</p>
			<form class="style-manager-notice-form" method="post">
				...
				<p>
					<button class="style-manager-notice-button button button-primary js-handle-style-manager">
						<span class="style-manager-notice-button__text"><?php esc_html_e( 'Yes, migrate customizations', 'style-manager' ); ?></span>
					</button>
					<button type="submit" class="style-manager-dismiss-button button button-secondary js-dismiss-style-manager"><?php esc_html_e( 'No, thank you', 'style-manager' ); ?></button>
			...
			(function ($) {
				$(function () {
					...
					$button.on('click', function (e) {
						...
						$buttonText.html("<?php esc_html_e( 'Migrating customizations..' ,'style-manager'); ?>")
						...
						$.ajax({
							...
							.done(function(response) {
								if (typeof response.success !== 'undefined' && response.success) {
									$statusMessage.html("<?php esc_html_e( 'Successfully migrated the parent customizations! Enjoy crafting your site!', 'style-manager' ); ?>")
									$buttonText.html("<?php esc_html_e( 'Finished migration', 'style-manager' ); ?>")
								} else {
									$statusMessage.html("<?php esc_html_e( 'Something went wrong and we couldn\'t migrate the customizations.' ,'style-manager'); ?>")
									$buttonText.html("<?php esc_html_e( 'Migration error' ,'style-manager'); ?>")
								}
							})
							.fail(function() {
								$statusMessage.html("<?php esc_html_e( 'Something went wrong and we couldn\'t migrate the customizations.' ,'style-manager'); ?>")
	...
	function migrate_customizations_from_parent_to_child_theme() {
		...
		if ( ! update_option( 'theme_mods_' . get_option( 'stylesheet' ), $new_theme_mods ) ) {
			wp_send_json_error( esc_html__( 'Could not update the child theme theme_mods.', 'style-manager' ) );
		}
	 ...
	 * @return array
	 */
	protected function filter_plugin_action_links_activate( $actions ): array {
	 ...
	 * @return array
	 */
	public function filter_plugin_action_links_deactivate( $actions ): array {
