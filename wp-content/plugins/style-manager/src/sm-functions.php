...
function sm_get_color_select_dark_config( $label, $selector, $default, $properties = [ 'color' ], $isDarker = false ): array {
	...
	$choices  = [
		'background' => esc_html__( 'Background', 'style-manager' ),
		'dark'       => esc_html__( 'Dark', 'style-manager' ),
		'accent'     => esc_html__( 'Accent', 'style-manager' ),
	];
	...
	if ( $isDarker ) {
		...
		$choices  = [
			'background' => esc_html__( 'Background', 'style-manager' ),
			'darker'     => esc_html__( 'Dark', 'style-manager' ),
			'accent'     => esc_html__( 'Accent', 'style-manager' ),
		];
	}
...
function sm_get_fallback_palettes(): array {
	...
	foreach ( $color_control_ids as $index => $control_id ) {
		...
		if ( $index === 0 ) {
			$label = __( 'Brand', 'style-manager' ) . ' ' . $label;
		} else {
			$label = ucfirst( $label );
		}
