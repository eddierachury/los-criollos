	...
	class NovaBlocks_Comments_Form {
		...
		public function __construct( $post = null, array $args = [] ) {
			...
			$this->args = wp_parse_args( $args, [
				...
				'commentLabel'                   => esc_html__( 'What\'s your comment or question?', 'nova-blocks' ),
				...
				'commentLabelKnownUser'          => esc_html__( 'What\'s your comment or question, %s?', 'nova-blocks' ),
				// Leave empty for no description.
				'commentDescription'             => esc_html__( 'Let\'s start a personal, meaningful conversation.', 'nova-blocks' ),
				// Leave empty for no placeholder.
				'commentPlaceholder'             => esc_html__( 'Share your knowledge or ask a question..', 'nova-blocks' ),
				...
				'commenterBackgroundLabel'       => esc_html__( 'What is your background around this topic?', 'nova-blocks' ),
				// Leave empty for no description.
				'commenterBackgroundDescription' => esc_html__( 'Example: Practical philosopher, therapist and writer.', 'nova-blocks' ),
				// Leave empty for no placeholder.
				'commenterBackgroundPlaceholder' => esc_html__( 'Your relevant background...', 'nova-blocks' ),

				'commenterNameLabel'       => esc_html__( 'What is your name?', 'nova-blocks' ),
				// Leave empty for no description.
				'commenterNameDescription' => '',
				// Leave empty for no placeholder.
				'commenterNamePlaceholder' => esc_html__( 'Ernest Hemingway', 'nova-blocks' ),

				'commenterEmailLabel'       => esc_html__( 'What is your email address?', 'nova-blocks' ),
				// Leave empty for no description.
				'commenterEmailDescription' => esc_html__( 'It will not be published or shared with others.', 'nova-blocks' ),
				// Leave empty for no placeholder.
				'commenterEmailPlaceholder' => esc_html__( 'your@email.com', 'nova-blocks' ),

				// Hide the commenter URL field by default.
				'commenterUrl'              => false,
				'commenterUrlLabel'         => esc_html__( 'What is your website URL?', 'nova-blocks' ),
				// Leave empty for no description.
				'commenterUrlDescription'   => '',
				// Leave empty for no placeholder.
				'commenterUrlPlaceholder'   => esc_html__( 'https://yourwonderfulsite.com', 'nova-blocks' ),

				'cookieConsentLabel' => esc_html__( 'Save my details in this browser for the next time I comment.', 'nova-blocks' ),

				// This is the default comment form submit button label.
				'submitLabel'        => esc_html__( 'Add this comment', 'nova-blocks' ),
				'cancelReplyLabel'   => esc_html__( 'Cancel reply', 'nova-blocks' ),
				// This is the label used on the submit button for the comment/reply form.
				'replyToText'        => esc_html__( 'Reply to %s', 'nova-blocks' ),
			] );
		}
			...
			if ( false !== strpos( $data_attributes['replyto'], '%s' ) ) {
				...
				if ( ! empty( $data_attributes['commentid'] ) ) {
					...
					if ( empty( $comment ) ) {
						...
						$data_attributes['replyto'] = ! empty( $args['submitLabel'] ) ? $args['submitLabel'] : esc_html__( 'Add this comment', 'nova-blocks' );
					} else {
...
<p class="comment-form-background comment-fields-wrapper">
	...
	<input id="nb_commenter_background" name="nb_commenter_background" type="text" value="%s" size="30" placeholder="%s" %s />
			...
			$defaults['cancel_reply_link'] = $args['cancelReplyLabel'] ?: esc_html__( 'Cancel reply', 'nova-blocks' );

			// Change the submit button
			$defaults['label_submit']  = $args['submitLabel'] ?: esc_html__( 'Add this comment', 'nova-blocks' );
