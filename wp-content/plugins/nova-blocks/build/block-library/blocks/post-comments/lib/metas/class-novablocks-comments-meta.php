	...
	class NovaBlocks_Comments_Meta {
		...
		public function verify_comment_meta_data( array $commentdata ): array {
			...
			if (empty($_POST['nb_commenter_background']) && $commenter_background_required) {
				$comment = new WP_Error( 'require_nb_commenter_background', __( '<strong>Error</strong>: You did not add your relevant background or experience.', 'nova-blocks' ), 200 );
			} else {
		...
		public function add_comment_meta_box() {
			...
			if ( $screen instanceof \WP_Screen && isset( $_GET['c'] ) && 'comment' === $screen->id ) {
				...
				if ( $comment && in_array( $comment->comment_type, [ 'comment' ], false ) ) {
					add_meta_box( 'nb_comment_extra_details', esc_html__( 'Extra Details', 'nova-blocks' ), [
							$this,
		...
		public function comment_meta_box_fields( WP_Comment $comment ) {
			...
			<fieldset class="widefat">
				<legend class="screen-reader-text"><?php esc_html_e( 'Comment extra details', 'nova-blocks' ); ?></legend>
				<table class="" role="presentation">
					<tbody>
						<tr>
							<td class="first"><label for="nb_commenter_background"><strong><?php esc_html_e( 'Commenter Relevant Background', 'nova-blocks' ); ?></strong></label></td>
