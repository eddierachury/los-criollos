...
if ( ! function_exists( 'anima_categories_posted_in' ) ) {
	function anima_categories_posted_in() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'anima' ) );
			if ( $categories_list ) {
				/* translators: before list of categories. */
				echo '<div class="cat-links"><span class="screen-reader-text">' . esc_html_x( 'Posted in', 'post categories', 'anima' ) . '</span>' . $categories_list . '</div>';
			}
		}
	}
}
...
<p class="has-text-align-right has-normal-font-size alignright">';
			...
			if ( empty( $hide_credits ) ) {
				$output .= '<span class="c-footer__credits">' . sprintf( esc_html__( 'Theme: %1$s by %2$s.', 'anima' ), esc_html( pixelgrade_get_original_theme_name() ), '<a href="https://pixelgrade.com/?utm_source=anima-clients&utm_medium=footer&utm_campaign=anima" title="' . esc_html__( 'The Pixelgrade Website', 'anima' ) . '" rel="nofollow">Pixelgrade</a>' ) . '</span>' . "\n";
			}
...
if ( ! function_exists( 'anima_footer_get_copyright_content' ) ) {
	...
	function anima_footer_get_copyright_content(): string {
		$copyright_text = apply_filters( 'anima/footer_copyright_text', esc_html__( '&copy; %year% %site-title%.', 'anima' ) );
		if ( ! empty( $copyright_text ) ) {
