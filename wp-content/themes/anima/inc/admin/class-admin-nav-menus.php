...
if ( ! class_exists( 'Anima_Admin_Nav_Menus', false ) ) :
	...
	class Anima_Admin_Nav_Menus {
		...
		public function __construct() {
			$this->menu_items_boxes_config = [
				'pxg-extras' => [
					...
					'menu_items' => [
						'search' => [
							'type'          => 'custom-pxg',
							'type_label'    => esc_html__( 'Custom', 'anima' ),
							// This is used for the default Navigation Label value once the menu item is added in a menu.
							'title'         => esc_html__( 'Search', 'anima' ),
							// This is the label used for the menu items list.
							'label'         => esc_html__( 'Search', 'anima' ),
							'url'           => '#search',
							'attr_title'         => esc_html__( 'Search for something you seek', 'anima' ),
							// These are classes that will be merged with the user defined classes.
							'classes'       => [ 'menu-item--search' ],
							'custom_fields' => [
								'visual_style' => [
									'type'        => 'select',
									'label'       => esc_html__( 'Visual Style', 'anima' ),
									'description' => esc_html__( 'Choose a visual style suitable to your goals and audience.', 'anima' ),
									'default'     => 'label_icon',
									'options'     => [
										'label'      => esc_html__( 'Label', 'anima' ),
										'icon'       => esc_html__( 'Icon', 'anima' ),
										'label_icon' => esc_html__( 'Label with icon', 'anima' ),
									],
								],
							],
						...
						'color-scheme-switcher' => [
							...
							'type_label'  => esc_html__( 'Custom', 'anima' ),
							// This is used for the default Navigation Label value once the menu item is added in a menu.
							'title'       => esc_html__( 'Light/Dark Mode', 'anima' ),
							// This is the label used for the menu items list.
							'label'       => esc_html__( 'Dark Mode Switcher', 'anima' ),
							'url'         => '#color-scheme-switcher',
							'attr_title'       => esc_html__( 'Switch between dark and light color mode', 'anima' ),
							// These are classes that will be merged with the user defined classes.
			...
			if ( function_exists( 'WC' ) && pixelgrade_user_has_access( 'woocommerce' ) ) {
				$this->menu_items_boxes_config['pxg-extras']['menu_items']['cart'] = [
					'type'        => 'custom-pxg',
					'type_label'  => esc_html__( 'Custom', 'anima' ),
					'title'       => esc_html__( 'Cart', 'anima' ),
					'label'       => esc_html__( 'Cart', 'anima' ),
					'url'         => esc_url( get_permalink( wc_get_page_id( 'cart' ) ) ),
					'attr_title'  => esc_html__( 'Toggle visibility of cart panel', 'anima' ),
					// These are classes that will be merged with the user defined classes.
			...
			<div id="anima-user-menu-links" class="taxonomydiv">
				...
				<span class="add-to-menu">
					<input type="submit" <?php disabled( $nav_menu_selected_id, 0 ); ?> class="button-secondary submit-add-to-menu right"
					       value="<?php esc_attr_e( 'Add to Menu', 'anima' ); ?>"
			...
			<div class="c-search-overlay sm-palette-1 sm-palette--shifted sm-variation-1">
				<div class="c-search-overlay__content">
                    ...
                    <div class="c-search-overlay__description">
	                    <?php esc_html_e( 'Begin typing your search above and press return to search.', 'anima' ); ?>
                        <span class="hide-on-mobile"><?php esc_html_e( 'Press Esc to cancel.', 'anima' ); ?></span>
                    </div>
		...
		public function custom_search_form( string $form ): string {
			return '<form role="search" method="get" class="wp-block-search__button-outside wp-block-search__text-button wp-block-search wp-block-search search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<label class="wp-block-search__label">
					<span class="screen-reader-text">' . _x( 'Search for:', 'label', 'anima' ) . '</span>
					<span class="search-icon"></span>
				</label>
				<div class="wp-block-search__inside-wrapper">
					<input type="search" class="wp-block-search__input search-field" placeholder="' . esc_attr( sprintf( __( 'Search %s...', 'anima' ), esc_html( get_bloginfo( 'name' ) ) ) ) . '" value="' . get_search_query() . '" name="s" />
                	<input type="submit" class="wp-block-search__button   search-submit" value="' . esc_attr_x( 'Search', 'submit button', 'anima' ) . '" />
				</div>
			</form>';
		}
		...
		public function add_badge_custom_field( int $item_id, WP_Post $item ) {
			...
			$field_classes = [ 'widefat', 'edit-menu-item-' . $name, ];
			$field_classes = implode( ' ', $field_classes );
			$field_label = __( 'Badge', 'anima' );
			$field_placeholder = __( 'e.g. New, Popular, Free, Sales', 'anima' );
			$field_description = __( 'A badge is <strong>a short text indicator</strong> used to inform visitors that something\'s special about this menu item.', 'anima' );
