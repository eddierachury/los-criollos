...
function anima_woocommerce_scripts() {
	...
	wp_localize_script( 'anima-woocommerce', 'pixelgradeWooCommerceStrings', [
		'adding_to_cart' => esc_html__( 'Adding...', 'anima' ),
		'added_to_cart' => esc_html__( 'Added!', 'anima' ),
	] );
...
function anima_woocommerce_sale_flash( string $markup ): string {
	return '<span class="wc-block-grid__product-onsale">' . esc_html__( 'Sale!', 'anima' ) . '</span>';
}
...
function anima_output_checkout_breadcrumbs() { ?>
    ...
    <ul class="woocommerce-checkout-breadcrumbs">
        <li>
            <a href="<?php echo esc_url( wc_get_cart_url() ); ?>"><?php esc_html_e( 'Cart', 'anima' ); ?></a>
        </li>
        <li><?php esc_html_e( 'Checkout', 'anima' ); ?></li>
    </ul>
        ...
        <div class="c-mini-cart">
            ...
            <div class="c-mini-cart__flyout">
                <div class="c-mini-cart__header">
                    <h5 class="c-mini-cart__title"><?php esc_html_e( 'Your cart', 'anima' ); ?></h5>
	...
	function woocommerce_display_categories() {
		...
		if ( ! empty( $terms ) ) {
			...
			if ( ! empty( $all_link ) ) {
				...
				echo '<li><a href="' . esc_url( $all_link ) . '" ' . ( ( ! isset( $current_term->term_id ) ) ? ' class="active"' : ' class="inactive"' ) . '>' . esc_html__( 'All Products', 'anima' ) . '</a></li>';
			}
...
if ( ! function_exists( ' anima_woocommerce_pagination_args' ) ) {
	function anima_woocommerce_pagination_args(): array {
		...
		return [
			...
			'mid_size'           => 2,
			'type'               => 'plain',
			'prev_text' => esc_html_x( 'Previous', 'previous set of posts', 'anima' ),
			'next_text' => esc_html_x( 'Next', 'next set of posts', 'anima' ),
		];
	}
}
...
if ( ! function_exists( 'anima_woocommerce_coupon_form' ) ) {
	function anima_woocommerce_coupon_form() {
		echo '<form class="checkout_coupon woocommerce-form-coupon" id="form-coupon" method="post" style="display:none">
                <input form="woocommerce-form-coupon" type="text" name="coupon_code" class="js-coupon-value-destination input-text" placeholder="' . esc_attr__( 'Coupon code', 'anima' ) . '" id="coupon_code" value="">
              </form>';
	}
}
...
function anima_woocommerce_breadcrumbs() {
	return [
		...
		'before'      => '<span>',
		'after'       => '</span>',
		'home'        => esc_html_x( 'Shop', 'breadcrumb', 'anima' ),
	];
}
...
function anima_woocommerce_quantity_label() {
	...
	$label = '<label for="quantity">' . esc_html__( 'Quantity', 'anima' ) . '</label><div class="quantity__wrapper">';
...
function anima_add_label_to_availability_display( array $availability ): array {
	...
	if( is_product() && $product-> get_manage_stock() ){
		$label = '<span>' . esc_html__( 'Stock', 'anima' ) . '</span>';
...
function anima_add_cart_to_extras_menu_items( $items ): array {
	...
	$items['pxg-extras']['menu_items']['cart'] = [
		...
		'label'       => esc_html__( 'WooCommerce Cart', 'anima' ),
