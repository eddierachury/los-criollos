...
function anima_pixcare_install_page() {
	 ...
	 */
	$admin_title = apply_filters( 'admin_title', esc_html__( 'Pixelgrade Care &rsaquo; Installer', 'anima' ), '' );
			...
			<div class="stepper">
				<div class="stepper__content">
					...
					<div class="section section--informative">
						...
						<h2 class="section__title u-text-center"><?php esc_html_e( 'One more lap to go..', 'anima' ); ?></h2>
						<div class="section__content">
							<?php
							echo sprintf(
								'%s <strong>%s</strong> %s',
								esc_html__( 'Start your digital journey with our', 'anima' ),
								esc_html__( 'Pixelgrade Care&#174;', 'anima' ),
								esc_html__( 'on board. It\'s the core of your experience with our WordPress theme, and we bet you are going to love it from day one. You\'re in great hands, no doubt about that!', 'anima' )
							); ?>
						</div>
						<div class="plugins">
							<div class="plugin  box  box--neutral  box--plugin-missing  box--plugin-invalidated">
								<div class="box__body">
									<h5 class="box__title"><?php esc_html_e( 'Pixelgrade Care&#174;', 'anima' ) ?></h5>
									<p class="box__text"></p>
								</div>
							</div>
						</div>
					...
					<a class="btn  btn--text  btn--return-to-dashboard"
					   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'anima' ) ?></a>
				</div>
			</div>
		</div>
	</div>
        ...
        $(function () {
		  ...
		  */
          if ( pluginStatus === 'missing' ) {
            $status.text("<?php esc_html_e( 'Installing...', 'anima' ) ?>");
            wp.ajax.settings.url = "<?php echo esc_url_raw( $install_url ) ?>"
          } else if ( pluginStatus === 'installed' ) {
            $staus.text("<?php esc_html_e( 'Activating...', 'anima' ) ?>");
          ...
          wp.ajax.send({type: 'GET'}).always(function (response) {
            ...
            installedSuccessfully = responseText.indexOf('<p><?php esc_html_e( 'Plugin installed successfully.', 'anima' ); ?></p>')
            ...
            folderAlreadyExists = responseText.indexOf('<p><?php esc_html_e( 'Plugin destination folder already exists.', 'anima' ); ?></p>')
            ...
            if (installedSuccessfully !== -1) {
              ...
              $status.text('<?php esc_html_e( 'Activating...', 'anima' ) ?>') ;
              ...
              wp.ajax.send({type: 'GET'}).always(function (response) {
                ...
                if (activatedSuccessfully !== -1 || noActionTaken !== -1) {
                  ...
                  $status.text('<?php esc_html_e( 'Unfortunately something went wrong. We are sorry, but you will need to install the plugin manually. Redirecting you to the appropriate page...', 'anima' ); ?>')
